...
| Browser-style terminal text zoom | Native shortcut mapping tests plus native app renderer/grid reconfiguration tests for increase, decrease, reset, shifted plus, modifier-wheel, and dedicated OS/browser zoom-key policy; `cargo run -- --runtime-text-zoom-smoke` verifies default 34/19/47 px metrics zoom to 39/22/54 px, reduces the visible grid, and resets without a live GPU window | Proven for covered controls |
