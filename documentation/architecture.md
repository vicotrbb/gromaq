...
- `config`: TOML settings, validation, defaults, theme tokens, and config-file
  reload state. The `tmux` config child module owns tmux enablement, status
  strip visibility, startup manager visibility, workspace-preset settings, and
  validates session, root, window, and pane command fields before launch code
  can consume them.
