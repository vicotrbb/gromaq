...
#[test]
fn tmux_manager_panel_dispatches_zoom_shortcut_to_selected_pane() {
    let snapshot = manager_snapshot();
    let mut panel = TmuxManagerPanelState::open_for_snapshot(&snapshot);
    let runner = FakeRunner::new(vec![vec!["resize-pane", "-Z", "-t", "%2"]]);
    ...
    assert_eq!(panel.last_action_feedback(), Some("zoom-pane success"));
}
