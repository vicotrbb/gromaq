...
fn runtime_tool_workflow_smoke_cli_reports_external_tool_pass_or_skip() {
    ...
    assert!(exit.stdout.contains("tools checked: 15"));
    ...
    assert!(exit.stdout.contains("top-snapshot:"));
    ...
    assert!(exit.stdout.contains("cargo-test:"));
    assert!(exit.stdout.contains("cargo-large-output:"));
