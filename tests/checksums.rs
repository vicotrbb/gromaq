...
mod unix {
    ...
    fn checksum_script_can_include_additional_release_assets() {
        ...
        fs::write(
            dist.path().join("gromaq-0.2.1-linux-x86_64.tar.gz"),
            b"stub archive",
        )
        .unwrap();
        ...
        assert!(
            manifest.contains("gromaq-0.2.1-linux-x86_64.tar.gz"),
    ...
    fn checksum_script_writes_custom_manifest_path() {
        ...
        assert!(contents.contains("gromaq-0.2.1-linux-x86_64.tar.gz"));
    }
