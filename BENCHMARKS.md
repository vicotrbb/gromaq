...
Native runtime render counters include rendered dirty regions/cells, sample count, total, average, max, and a bounded bucketed p95 estimate for rendered dirty frames. The runtime also tracks app-input-to-render latency with bounded sample, total, average, max, and p95 counters. `cargo run -- --runtime-perf-smoke` on 2026-06-23 pumped 1 byte through a deterministic PTY echo, rendered 1 CPU-side frame, and reported rendered dirty-region work plus render and input-to-render sample, average, max, and p95 counters. `cargo run -- --runtime-real-shell-perf-budget-smoke` applies the same 6.94 ms render p95 gate but a looser 20 ms input-to-render p95 gate (real-shell end-to-end timing includes OS shell-response variance; the strict 10 ms terminal-latency target is still enforced by the deterministic `--runtime-perf-budget-smoke`) to a real `/bin/sh` PTY transcript. Large, bounded-state, continuous-output, and local scrollback runtime smokes validate nonzero rendered dirty-region/cell work and cap each rendered dirty-cell batch to the visible viewport; `cargo run -- --runtime-real-shell-large-output-smoke` additionally enforces the 6.94 ms render p95 gate while proving bounded scrollback eviction for real `/bin/sh` large output. `cargo run -- --runtime-idle-smoke` on 2026-06-23 pumped 0 bytes, reported 16 render attempts, 16 clean-frame skips, and 0 rendered frames through the deterministic CPU-side path. Those counters are structured inputs for future live-window proof, but they do not by themselves prove hardware-backed 144Hz p95 frame time, full live input-to-present latency, or idle CPU.
...
Render-plan generation is unit-tested against dirty-region and full-viewport modes and benchmarked for CPU-side command generation. Glyph-quad generation is integration-tested for pixel positions, wide-cell geometry, atlas UVs, and triangle indices, and benchmarked both directly and through prepared terminal glyph-frame construction. The offscreen textured-quad and terminal-text smoke tests prove sampled draw pipelines and readback, and `cargo run -- --gpu-terminal-text-snapshot <path>` writes the same contrast-gated terminal-text draw/readback frame as an inspectable PPM artifact. The Criterion harness includes `gpu_textured_quad_draw_readback` plus `gpu_terminal_text_draw_readback` for live offscreen GPU draw/readback measurement when a compatible adapter is available; they still do not prove windowed terminal frame time.
