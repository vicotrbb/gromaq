...
Native runtime render counters include sample count, total, max, and a bounded bucketed p95 estimate for rendered dirty frames. The runtime also tracks app-input-to-render latency with bounded sample, total, max, and p95 counters. Those counters are structured inputs for future live-window proof, but they do not by themselves prove hardware-backed p95 frame time or input latency.
