...
- ANSI SGR colors/text attributes including slow/rapid blink, hidden, overline, colon-form underline styles, and SGR underline color/reset; cursor movement, erase-line, erase-display, DEC autowrap/origin modes, and RIS full reset handling
- VT editing/navigation subset: default and configurable tab stops, C0 LF/VT/FF linefeed controls, DEC special graphics G0/G1 box-drawing charsets with SI/SO shifting, DECALN screen alignment pattern, cursor forward/backward tabs, insert/delete/erase/repeat characters, insert/replace mode, insert/delete lines with scroll-margin bounds, viewport scroll up/down with scroll-margin bounds, DECSTBM linefeed, index/next-line, and reverse-index scroll margins, cursor absolute/relative row and column positioning including DEC origin mode, cursor next/previous line movement, cursor visibility and DECSCUSR shape/blink state, DEC cursor/rendition save/restore, and SCO/private-mode cursor save/restore
- Terminal-generated Primary/Secondary DA, DECID, and DSR status/cursor-position replies with native PTY write-back
- OSC title handling, OSC 8 hyperlink cell metadata, and bounded OSC 52 clipboard-text decoding
- Bracketed paste mode encoding
- Unicode wide-cell handling, including combining marks attached to wide glyphs
- Bounded scrollback
...
- Keyboard input encoding for common keys, navigation keys, modified named keys, F1-F12 keys, application cursor-key mode, focus reports, committed platform text, paste payloads, native clipboard paste shortcuts, mouse reports, and native `winit` key events
