...
use help::{render_help_catalog, render_help_catalog_action_coverage};
...
use render::{
    STARTUP_MANAGER_SMALL_GRID_COLS, STARTUP_MANAGER_SMALL_GRID_ROWS, render_current_pane_marker,
    render_current_target_pane_detail, render_current_target_row_markers,
    render_manager_header_status, render_manager_panel, render_manager_panel_contains,
    render_manager_state, render_no_server_start_hint, render_outside_attach_hint,
    render_outside_attach_target, render_startup_manager_after_shell_prompt,
    render_status_feedback, render_status_guidance, render_status_pane_command,
    render_status_strip,
};
use report::runtime_tmux_ui_smoke_result;
use shortcuts::{
    drive_attach_session_handoff, drive_destructive_shortcut_confirmation,
    drive_detach_session_failure, drive_detach_session_shortcut, drive_kill_pane_confirmation,
    drive_kill_window_confirmation, drive_missing_tmux_feedback, drive_name_entry_action,
    drive_new_window_shortcut, drive_refresh_preserves_action_focus, drive_refresh_shortcut,
    drive_rename_session_action, drive_rename_window_action, drive_select_pane_shortcut,
    drive_split_down_shortcut, drive_split_right_shortcut, drive_start_session_feedback,
    drive_start_session_pty_handoff, drive_unavailable_shortcut_block,
    drive_window_cycle_shortcuts, drive_zoom_shortcut, drive_zoom_toggle_shortcut,
};
...
pub(in crate::cli) fn runtime_tmux_ui_smoke_exit() -> CliExit {
    ...
    let status_guidance_checked = render_status_guidance(&mut renderer);
    ...
    let manager_header_status_checked = render_manager_header_status(&mut renderer);
    ...
    let current_target_row_markers_checked = render_current_target_row_markers(&mut renderer);
    ...
    let outside_attach_target_checked = render_outside_attach_target(&mut renderer);
    let help_catalog_checked = render_help_catalog(&snapshot, &mut renderer);
    let help_catalog_action_coverage_checked =
        render_help_catalog_action_coverage(&snapshot, &mut renderer);
    ...
    let detach_session_shortcut_checked = drive_detach_session_shortcut(&snapshot);
