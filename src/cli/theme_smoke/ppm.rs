pub(super) fn ppm_bytes(width: u32, height: u32, pixels: &[u8]) -> Result<Vec<u8>, String> {
    ...
    if pixels.len() != expected_rgba_len {
        return Err(format!(
            "snapshot expected {expected_rgba_len} RGBA bytes, got {}",
            pixels.len()
        ));
    }
    let rgb_len = usize::try_from(u64::from(width) * u64::from(height) * 3)
        .map_err(|_| "snapshot RGB buffer is too large".to_owned())?;
    let header = format!("P6\n{width} {height}\n255\n");
    let mut snapshot = Vec::new();
    snapshot
        .try_reserve_exact(header.len() + rgb_len)
        .map_err(|_| "snapshot allocation failed".to_owned())?;
