...
#[test]
fn prepared_surface_glyph_frame_preview_preserves_cell_spacing_gaps() {
    let plan = RenderPlan {
        viewport_cols: 2,
        viewport_rows: 1,
        cursor: CursorSnapshot {
    ...
    assert_eq!(
        preview_pixel(&preview.rgba, preview.width, 3, 0),
        [0, 0, 0, 255]
    );
    assert_eq!(
        preview_pixel(&preview.rgba, preview.width, 4, 0),
        [80, 90, 100, 255]
    );
}
